% Imputation using the Euclidean distance function.
% Finds minimun Euclidean distance between previous naumber of NaN
% and other column elements, and replaces NaN with next element
% of the element with minimun Euclidean distance.
...
for col=1:length
    for row =1:height
        if (nan_values(row, col) == 1)
            if row==1
                [row3,col3] = first_non_nan_col(row, col, x, height);
                x(row,col) = x(row3,col3);
            else
                min = 1;
                mins_row = 0;
                mins_col = 0;
                for row2=1:height
                    ...
                    %col
                    check = 0;
                    if  pdist2(x(row-1,col),x(row2,col)) < min & row2 ~= row-1 & nan_values(row2,col) == 0
                        for row3=row2+1:height
                            if nan_values(row3, col)==0
                                check = 1
                                break;
                            end
                        end
                        if(check == 1)
                            min = pdist2(x(row-1,col),x(row2,col))
                            mins_row = row2
                            mins_col = col
                            if(row2 == height)
                                x(row,col) = x(row2, col);
                                continue;
                            end
                        end
                    end
                    [row2,col2] = first_non_nan_col(mins_row, col, x, height);
