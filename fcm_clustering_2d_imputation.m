% 2-D imputation method using Fuzzy c-means algorithm.
...
for col=1:length
    ...
    for row =1:height
        if (nan_values(row, col) == 1)
            ...
            [centers,U] = fcm(x(1:row-1,col),3); % for 3 clusters
            
            [~,idc] = max(U);
            temp_array = [idc(1,row-3) idc(1,row-2) idc(1,row-1)]; % window = 3

            search_area = idc(1, 1:row-initial_window-1);
            k = strfind(search_area,temp_array);
            k = k + initial_window;
            times_found = numel(k);
            if(times_found == 0 || times_found == 1)
                new_window = 2;
                temp_array = [idc(1,row-2) idc(1,row-1)];
                search_area = idc(1,1:row-new_window-1);
                k = strfind(search_area,temp_array);
                k = k + new_window;
                times_found = numel(k);
                if(times_found == 0 || times_found == 1)
                    new_window = 1;
                    temp_array = [idc(1,row-1)];
                    search_area = idc(1,1:row-new_window-1);
                    k = strfind(search_area,temp_array);
                    k = k + new_window;
                    times_found = numel(k);
...
%Timeseries comparison between initial and completed dataset
