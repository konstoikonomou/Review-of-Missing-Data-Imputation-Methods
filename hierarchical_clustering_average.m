% 2-D imputation method using hierarchical clustering method.
...
for col=1:length
    ...
    for row =1:height
        if (nan_values(row, col) == 1)
            ...
            Z = linkage(x(1:row-1,col),'average');
            c = cluster(Z,'Maxclust',3); % setting the # of clusters
            
            temp_array = [c(row-3,1) c(row-2,1) c(row-1,1)]; % window = 3

            search_area = c(1:row-initial_window-1,1);
            k = strfind(search_area',temp_array);
            k = k + initial_window;
            times_found = numel(k);
            if(times_found == 0 || times_found == 1)
                new_window = 2;
                temp_array = [c(row-2,1) c(row-1,1)];
                search_area = c(1:row-new_window-1,1);
                k = strfind(search_area',temp_array);
                k = k + new_window;
                times_found = numel(k);
                if(times_found == 0 || times_found == 1)
                    new_window = 1;
                    temp_array = [c(row-1,1)];
                    search_area = c(1:row-new_window-1,1);
                    k = strfind(search_area',temp_array);
                    k = k + new_window;
                    times_found = numel(k);
...
mape_array = initial - x;
mape_array = mape_array ./ initial;
mape_array = abs(mape_array);
mape_array = mape_array';
mape_per_day = mean(mape_array,1)
[min_mape, day_with_min_mape]=min(mape_per_day)
[max_mape, day_with_max_mape]=max(mape_per_day)

a = x(day_with_min_mape,:);
ts1 = timeseries(a);
figure
plot(ts1, '-k', 'MarkerSize', 6)
grid on
hold on
b = initial(day_with_min_mape,:);
ts2 = timeseries(b);
plot(ts2, '--r','MarkerSize', 6 )
legend('Completed', 'Actual')
