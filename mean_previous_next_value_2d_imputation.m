...
% with the mean of previous and next value of the column
...
for col=1:length;
    for row=1:height;
        if(nan_values(row,col) == 1)
            ...
            elseif row == height
                [row2, col2] = last_non_nan_col(row, col, x, height);
                x(row, col) = x(row3, col3);
            else
                [row2, col2] = last_non_nan_col(row, col, x, height);
                [row3, col3] = first_non_nan_col(row, col, x, height);
%                 x(row,col) = (x(row-1,col) + x(row+1,col)) / 2;
                x(row,col) = (x(row3,col3) + x(row2,col2)) / 2;
            end
        end
    end
end
